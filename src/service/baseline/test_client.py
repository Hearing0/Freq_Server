...
SHM_SIZE = 1024
SEM_SERVER = "/sem_server"                  # For Syncronization 
...
def initialize_semaphores():
    """ Initializes Syncronization Semaphores. Attempts to check for already 
    ...
    try:
        ...
        with mmap.mmap(shm_fd, SHM_SIZE, mmap.MAP_SHARED, mmap.PROT_READ | mmap.PROT_WRITE) as m:
            while True:    
                ...
                sem_client.acquire()
                print("[Frequency Client] Aquired Client Request...")
                
                # Read data from shared memory object
                print("[Frequency Client] Reading data from Shared Memory...")
                m.seek(0)
                data = m.read(SHM_SIZE).rstrip(b'\x00')
                print(f"[Frequency Client; from Server] {data.decode()}")
