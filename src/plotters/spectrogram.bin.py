...
# TODO: Make dynamic based on file contents
num_samples = 5000
...
if len(fft_files) == 0:
    print("No FFT files found for the specified date. Exiting.")
    exit(1)
...
# Display plot
plt.tight_layout()
plot_name = f"spectrogram.{args.stid}.{date_str}"
plot_name += f"_{args.time_start.strftime('%H%M%S')}-{args.time_end.strftime('%H%M%S')}"
plot_name += ".png"
SAVE_PLOT_AS = os.path.join("plots/debug", plot_name)
